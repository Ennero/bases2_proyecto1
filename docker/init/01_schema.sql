...
CREATE TABLE IF NOT EXISTS aparicion_partido (
    ...
    seccion text NOT NULL CHECK (
        seccion IN ('titular', 'ingresado', 'suplente_no_jugo')
    ),
...
CREATE
OR REPLACE VIEW v_evento_jugador_pendiente AS
SELECT
    'gol' :: text AS source_table,
...
FROM
    gol g
    LEFT JOIN resolucion_identidad_jugador r ON r.source_table = 'gol'
    AND r.source_event_id = g.gol_id
WHERE
    g.jugador_id IS NULL
UNION
ALL
SELECT
    'tarjeta' :: text,
...
FROM
    tarjeta t
    LEFT JOIN resolucion_identidad_jugador r ON r.source_table = 'tarjeta'
    AND r.source_event_id = t.tarjeta_id
WHERE
    t.jugador_id IS NULL
UNION
ALL
SELECT
    'cambio_entrada' :: text,
...
FROM
    cambio c
    LEFT JOIN resolucion_identidad_jugador r ON r.source_table = 'cambio_entrada'
    AND r.source_event_id = c.cambio_id
WHERE
    c.jugador_entra_id IS NULL
UNION
ALL
SELECT
    'cambio_salida' :: text,
...
FROM
    cambio c
    LEFT JOIN resolucion_identidad_jugador r ON r.source_table = 'cambio_salida'
    AND r.source_event_id = c.cambio_id
WHERE
    c.jugador_sale_id IS NULL
UNION
ALL
SELECT
    'penal' :: text,
...
FROM
    penal p
    LEFT JOIN resolucion_identidad_jugador r ON r.source_table = 'penal'
    AND r.source_event_id = p.penal_id
WHERE
    p.jugador_id IS NULL;
